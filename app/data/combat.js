...
EN.combat = {
  ...
  destructibleCover: "Destructible Cover\n\nCover in Elysium is not permanent. Sustained fire chews through it, explosives erase it, and the wall that saved you last round can be rubble by the next.\n\nEach discrete section of solid cover (roughly a 1 space section that is actually protecting someone) has two values:\n- Structure: a damage threshold.\n- Integrity: how much punishment that section can take before it is destroyed.\n\nA body wider than one space can be behind more than one section. Each is tracked on its own, and losing one drops that body a cover grade instead of stripping its cover outright.\n\nStructure\n- If an attack's damage is less than Structure, that section takes no damage from that hit.\n- If damage is equal to or higher than Structure, that section takes the full rolled damage to its Integrity.\n\nIntegrity\n- When Integrity reaches 0, that section is destroyed and no longer provides cover.\n- The wreckage can linger as scenery, but it grants no mechanical Cover.",

  /* The general rule that closes the "my Invocation says it melts things, so it melts
     things" exploit. It governs EVERY effect, not one ability: one damage path for
     objects whatever the source, a stated material ceiling for effects that deal no
     damage, and armor as DR only. The last line is the override switch, so a feature
     bypasses this only where its own text says so. Sits between Integrity and Overflow
     Damage in print, and sits between them here. */
  effectsAndObjects: "Effects and Objects\n\nEffects do not automatically destroy objects just because their description says they burn, melt, dissolve, shatter, or otherwise damage them. If an effect deals damage, resolve it against an object the same way you would any other source of damage. If the damage equals or exceeds the object's Structure, the object takes the full rolled damage to Integrity. If it does not meet the Structure threshold, the object takes no damage.\n\nIt does not matter whether that damage came from a shotgun, an explosive, or a Flow Invocation. Objects use the same rules for all of them.\n\nIf an effect does not deal damage, its description instead states the highest material category it can destroy, or names the specific materials it works on. It can affect objects of that category or lower, but not anything tougher. A corrosive gel rated to eat through Cheap materials, for example, cannot burn through a Fortified door no matter how long it is applied.\n\nArmor works differently. Armor has no Structure or Integrity; it only has DR. If an effect damages or degrades armor, it reduces that DR by the amount stated in the effect until the armor is repaired.\n\nA feature only overrides these rules if its description specifically says that it does.",

  /* Vehicles map onto the table above by their own Structure, which was undefined until
     2026-08-24. The mapping is free: every printed vehicle Structure is already a Cover
     Material value. Reference prose, because the app does not model cover as a live
     tracker for anything, vehicles included; see DEFERRED-FIXES. */
  vehiclesAsCover: "Vehicles as cover\n\nA Freelancer sheltering behind a vehicle uses the Cover Material Table as normal. Use the vehicle's Structure to determine the row: 7 Cheap, 9 Average, 11 Heavy, 14 Fortified, 18 Hardened. An Armored vehicle always counts as Hardened because that trait sets its Structure to 18.\n\nThe cover section uses that row's Integrity, not the vehicle's full Integrity. A Cargo Hauler might have 80 Integrity as a vehicle, but the panel you are crouched behind has only 18 Integrity as cover.\n\nAny damage dealt to that cover section also comes off the vehicle's own Integrity. One hit, both ledgers. You can hide behind the sedan all afternoon; the sedan will remember.\n\nA trait can specify a different Cover Material row for part of a vehicle. For example, a braced Walker gives allies behind its legs Heavy cover, one step below the Hardened hull beside them.",
  ...
  /* How far Reach can be pushed. A weapon's own Reach plus everything a character
     ...
     cap strikes 3 spaces (1 adjacent + 2) and a flexible one strikes 4.
     In the manuscript as of 2026-08-11, in the Reach section of Part 2 and again in
     the Reach X gear trait: "If a part, mod, or effect would increase a weapon's Reach
     beyond its cap, its Reach increases only to that cap and any excess is lost. A
     feature can exceed this cap only if its own text specifically says so."
     Exactly one feature says so today, Canopy Reach, and the engine applies it after
     the cap for that reason. */
  reachCap: { rigid: 2, flexible: 3 },
  reachCapText: "Reach caps by construction: Reach 2 for rigid weapons (blades, blunt weapons, anything mounted on a shaft) and Reach 3 for flexible weapons (whips, cables, filament lines). A part, mod, or effect that would push past the cap raises Reach only to the cap and the excess is lost. A feature can exceed the cap only if its own text says so.",

  shieldDurability: "A shield has Durability boxes, usually 3, though some gear may have more or fewer. Its Wear Threshold equals twice the maximum result of its Block die: 8 for a 1d4 shield, 12 for a 1d6 shield, or 16 for a 1d8 shield.\n\nWhenever you Block an attack whose raw damage equals or exceeds the shield's Wear Threshold, mark off 1 Durability box. Raw damage is the attacker's rolled damage before applying your Block or Armor DR. A critical hit always marks off 1 box when Blocked, regardless of its damage.\n\nDamage below the Wear Threshold does not wear down the shield; absorbing smaller hits is what it was built to do. As long as the shield has at least 1 Durability box remaining, it can be restored through a normal Repair Project.\n\nAt 0 boxes, the outcome depends on the shield:\n- Physical: the shield is destroyed. It grants no Defense bonus, no Block, and no Cover, and is beyond repair, but the wreck counts as salvage (see Crafting and Projects) that can reduce or eliminate the material cost of a later repair or build.\n- Emitter or hardlight: the boxes track projector overload rather than physical wear, so it goes dark instead. It grants nothing until it resets or is repaired. It is not destroyed and leaves no salvage.",
  damagePipeline: "When a Target is targeted by an attack or damaging effect, follow these four steps in order:\n\n1. Determine the Hit (Avoidance)\n- Compare the incoming Attack Roll to the Target's Defense score (Cover is already calculated into the base score).\n- Evasion: At this exact moment, the Target may spend an Impulse Action to Dodge, retroactively boosting their Defense score. If this causes the attack to miss, the pipeline ends immediately.\n- If the attack roll still exceeds the final Defense score, it hits. The raw damage is the weapon's dice plus the same attribute modifier the attack roll used.\n- Indirect delivery (a grenade, a launcher shell) adds no attribute modifier, and neither do cipher, Flow, or Tech effects unless their own text grants one.\n\n2. Active Mitigation (Impulse)\n- If the attack hits and the Target hasn't already used a defensive Impulse against this Enemy, they may spend one now to use Block, Parry, Ward, Resurge, Siphon, or Brace.\n- Roll the defense's corresponding die and subtract the result, plus any flat bonuses the defense carries (a suit's Block Bonus, the Plated trait), directly from the raw incoming damage. Resolve any secondary effects of the defense immediately (such as Siphon restoring Vigor, or Resurge dealing rebound damage).\n\n3. Passive Mitigation (Armor DR)\n- Subtract the defending Target's Armor Damage Reduction (DR) from the remaining damage.\n- Some damage types (like Force (Spatial) or Psychic) may ignore standard physical DR entirely.\n\n4. Apply Damage (Depletion)\n- Apply the final damage total to the Target's health pools in the following order:\n- Vigor: Subtract damage from Vigor first.\n- Vitality: Any leftover damage spills directly into Vitality.\n- Wounds: If Vitality is reduced to 0, the Target suffers a Wound. Apply the leftover damage to the Wound track and check for severe conditions.",
