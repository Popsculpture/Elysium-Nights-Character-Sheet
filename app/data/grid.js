...
EN.grid = {
  ...
  buddyCiphers: [
    { name: "Node Sweeper", type: "Utility", activation: "1 Action", range: "Self (Area 6 radius)", text: "Sweep the area for devices and nodes. Automatically reveals every hidden node whose Scan DC is 12 + (3 × the Buddy's Tier) or lower: Obscured for a Standard [0] Buddy, Masked for an Improved [1], Stealth-Routed for an Advanced [2]. More deeply hidden nodes still require an active Systems (Tech) scan against their Scan DC. No Cipher Save is involved." },
    { name: "Access Spike", type: "Breach", activation: "1 Action", range: "12 spaces", text: "Cipher Attack vs. node Security Rating. On hit, establish a Link (lasts 8 hours)." },
    { name: "Access Override ⇋", type: "Utility", activation: "1 Action", range: "Touch (linked)", text: "• **Fail:** Lock or unlock the mechanism. * **Success:** Lock resists; IC retaliates." },
    { name: "Hijack Stream ⇋", type: "Detection", activation: "1 Action", range: "12 spaces", text: "• **Fail:** Clone or reroute the feed. * **Success:** Access denied; IC retaliates." },
    { name: "Data Probe ⇋", type: "Detection", activation: "1 Action", range: "Self (via Link)", text: "Probe a linked node for files/logs/configs. Grants Edge on d20 (or +1 Edge Die) for Investigation or Systems searches." },
    { name: "Data Extraction ⇋", type: "Utility", activation: "1 Swift Action", range: "Touch", text: "• **Fail:** Clean copy, no trace. * **Success:** You still get the data, but IC retaliates and leaves a forensic trail." }
  ],
...
/* `activation` is the action cost, and it was `exec` until 2026-09-18. Part 2 renamed the
   printed label from "Execution:" to "Activation:" because it collided with the Operator's
   class resource, which is also called Execution and is unchanged. The app never printed the
   old label, so nothing visible was wrong, but the key now matches both the book and the gear
   side, where `it.activation` has always been the action-cost field (inventory.js renders it
   under an "Activation:" heading). One name for one idea across both halves of the catalog.
   Never persisted: a save stores cipher NAMES, so this rename carries no migration. */
EN.grid.ciphers = [
  /* ---- Complexity 1 · Improved ---- */
  { name: "Logic Bomb", cx: 1, cat: "Offense", sub: "Combat", activation: "1 Action", range: "12 spaces", runtime: "Instant", signature: true,
    text: "Inject malicious code into a target node. Cipher Attack vs. Security Rating. On hit, roll **2d6 damage**. Subtract the node's Firewall Damage Threshold (if any); the remainder comes off its System Integrity. **Signature:** Costs 1 Bandwidth regardless of Complexity." },
  { name: "Shrapnel Code", cx: 1, cat: "Offense", sub: "Combat", activation: "1 Action", range: "Area 6 cone", runtime: "Instant",
  ...
  { name: "Hardline Tap", cx: 1, cat: "Offense", sub: "Breach", activation: "1 Action", range: "Touch", runtime: "Link lasts 8 hours", link: true,
  ...
  { name: "Spoof Persona", cx: 1, cat: "Manipulation", sub: "Utility", activation: "1 Action", range: "Self (via Link)", runtime: "10 minutes", link: true,
  ...
  { name: "Cold Boot", cx: 1, cat: "Manipulation", sub: "Control", activation: "1 Action", range: "Touch (Linked)", runtime: "Instant", link: true,
  ...
  { name: "Tracker Scrub", cx: 1, cat: "Manipulation", sub: "Utility", activation: "1 Action", range: "Self (via Link)", runtime: "Instant", link: true,
  ...
  { name: "Hotwire", cx: 1, cat: "Manipulation", sub: "Control", activation: "1 Action", range: "Touch (Linked)", runtime: "Instant", link: true,
    text: "The street version of taking the controls, and the first taste of a protocol you will spend a career perfecting. Force a Linked device to perform a single basic action you dictate, right now. Node rolls Cipher Save vs. your DC. On failure, the device does one thing it is built to do and you have the authority to command: a turret takes one shot at a target you pick, a door cycles, a parked car lurches forward a few spaces, a drone moves its Speed. One command, then your grip slips and the device belongs to its owner again. On success, the command is refused and IC retaliates." },
  { name: "Live Tap", cx: 1, cat: "Manipulation", sub: "Utility", activation: "1 Action", range: "Self (via Link)", runtime: "1 scene", link: true,
    text: "Where Hijack Stream grabs a feed and runs, Live Tap settles in to listen. Open a quiet channel into a Linked node carrying audio or video and ride it live for the rest of the scene. Node rolls Cipher Save vs. your DC. On failure, you see and hear everything the node does, in real time, hands free, while you get on with other work. On success, the feed catches you in it and Alert triggers. The tap rides one node. Move it and you cast again." },
  { name: "Firewall Patch", cx: 1, cat: "Protection", sub: "Defensive", activation: "1 Swift Action", range: "Touch (Linked friendly node)", runtime: "1 scene", link: true,
  ...
  { name: "Bounce Routing", cx: 1, cat: "Protection", sub: "Defensive", activation: "1 Impulse Action", range: "Self", runtime: "Instant",
  ...
  { name: "Daisy Chain", cx: 2, cat: "Offense", sub: "Breach", activation: "1 Action", range: "Self (via existing Link)", runtime: "Persistent", link: true, signature: true,
    text: "From a Linked node, automatically establish a new Link to any node directly networked to it. The new Link costs no additional Cipher Attack roll, but counts against your maximum Links. Cannot chain into a Hardened Node. **Signature:** Costs 1 Bandwidth regardless of Complexity." },
  { name: "Brute Force", cx: 2, cat: "Offense", sub: "Combat", activation: "1 Action", range: "12 spaces", runtime: "Instant",
  ...
  { name: "Smartgun Hijack", cx: 2, cat: "Offense", sub: "Combat", activation: "1 Action", range: "12 spaces", runtime: "1 round", link: true,
    text: "You graduate from one stolen trigger pull to holding the weapon outright. Target a Linked smartgun, smartweapon, or networked turret. Node rolls Cipher Save vs. your DC. On failure, the weapon is slaved to you until the start of your next turn: it fires at targets you choose, using its own attack bonus, and it will not turn on your crew no matter who has hands on it. Its owner can try to wrest it back on their turn with a contested Systems check, and until they win that contest the gun answers to you. On success, the weapon shakes off the intrusion and IC retaliates." },
  { name: "Feedback Loop", cx: 2, cat: "Manipulation", sub: "Control", activation: "1 Action", range: "Touch (Linked)", runtime: "1 scene", link: true,
  ...
  { name: "Puppet Vehicle", cx: 2, cat: "Manipulation", sub: "Control", activation: "1 Action", range: "Touch (Linked vehicle)", runtime: "1 minute", link: true,
  ...
  { name: "Echo Capture", cx: 2, cat: "Manipulation", sub: "Utility", activation: "1 Swift Action", range: "Self (via Link)", runtime: "Instant", link: true,
  ...
  { name: "Blackout", cx: 2, cat: "Manipulation", sub: "Control", activation: "1 Action", range: "Touch (Linked grid node)", runtime: "1 scene", link: true,
  ...
  { name: "Decoy Persona", cx: 2, cat: "Protection", sub: "Defensive", activation: "1 Action", range: "Self (via Link)", runtime: "1 hour", link: true,
  ...
  { name: "Glitchstorm", cx: 3, cat: "Offense", sub: "Combat", activation: "1 Action", range: "12 spaces, Area 4 sphere", runtime: "Instant",
  ...
  { name: "Cascade Worm", cx: 3, cat: "Offense", sub: "Breach", activation: "1 Action", range: "Self (via Link)", runtime: "Persistent", link: true,
  ...
  { name: "Puppet String", cx: 3, cat: "Manipulation", sub: "Control", activation: "1 Action", range: "Touch (Linked)", runtime: "1 minute", link: true, signature: true,
    text: "The whole hand on the strings, not one finger on a trigger. Seize direct control of any Linked node's primary function: a weapon, a vehicle, a lock, a drone, a fabricator, or the strange one-off device the lower ciphers never had a name for. Node rolls Cipher Save vs. your DC. On failure, you operate the device as its authorized operator for the full minute, issuing whatever commands it can take, not a single action but the run of it (drive the car and work its systems, walk the turret and fire it, seal a room and decide whose badge its locks still answer to). Because you are rooted this deep, the owner wrests back with **Snag** on the contested Systems check, and must spend their Action to make the attempt. On success, the node throws you out and IC retaliates. **Signature:** Costs 1 Bandwidth regardless of Complexity." },
  { name: "Identity Mirror", cx: 3, cat: "Manipulation", sub: "Utility", activation: "1 Action", range: "Touch (Linked)", runtime: "1 scene", link: true,
  ...
  { name: "Dead Zone", cx: 3, cat: "Manipulation", sub: "Control", activation: "1 Action", range: "Touch (Linked node)", runtime: "1 scene", link: true,
    text: "Turn a Linked node into a wall of noise across every wireless band. For the rest of the scene, an Area 4 zone around the node is dead air. No new wireless Link forms inside it, and any hostile wireless Link already reaching in (an enemy hacker's, a rigged drone's, a smartgun's targeting feed) must succeed on a Cipher Save vs. your DC at the start of each of its owner's turns or drop. Networked weapons and remote drones caught inside go inert until they leave. You and allies you name are cut a clean channel through the noise; your own signatures pass. Wired Links (Hardline Tap) ignore the zone on both sides. The hard answer to a rigger who thought distance kept them safe." },
  { name: "Phantom Shell", cx: 3, cat: "Protection", sub: "Defensive", activation: "1 Swift Action", range: "Self", runtime: "1 round", link: true,
  ...
  { name: "Hotpatch", cx: 3, cat: "Protection", sub: "Defensive", activation: "1 Action", range: "Touch (Linked friendly node or device)", runtime: "Instant", link: true,
    text: "You don't only break nodes. Sometimes you keep one breathing. Restore **System Integrity equal to 5 × your Tech Modifier (minimum 5)** to a Linked allied node or device (a Smartdeck, B&E Buddy, or drone). This cannot bring back something already at zero: Bricked nodes and devices still require standard repair. Usable once per scene per target." },

  /* ---- Complexity 4 · Elite ---- */
  { name: "System Cascade", cx: 4, cat: "Offense", sub: "Combat", activation: "1 Action", range: "Self (via Link)", runtime: "Instant", link: true, signature: true,
    text: "Detonate corrupted code through a Linked node and into every node it is networked to. The primary node takes **5d6 damage**. Every node directly networked to it takes **3d6 damage**, rolled once and applied to each. Both resolve as normal cipher damage: subtract each node's own Firewall Damage Threshold, and the remainder comes off its System Integrity. **Signature:** Costs 1 Bandwidth regardless of Complexity." },
  { name: "Backtrace", cx: 4, cat: "Offense", sub: "Breach", activation: "1 Action", range: "Self (via Link)", runtime: "Instant", link: true,
    text: "Someone is working against you in the wire. Work back harder. When a hostile operator is reaching into a node you are Linked to (a Guardian on the floor, an enemy Codebreaker sharing the space, a trace you have spotted), choose one: pin their physical location and hand it to your crew for the scene, turning a Guardian's favorite escalation back on them; or force that operator to make a Cipher Save vs. your DC and, on a failure, sever one of their active Links and deal them the standard LinkDeath feedback for the dropped connection. Against a Guardian, a severed Link buys seconds. Seconds are sometimes the whole job." },
  { name: "Ghost in the Machine", cx: 4, cat: "Manipulation", sub: "Utility", activation: "1 Action", range: "Self (via Link)", runtime: "1 hour", link: true,
  ...
  { name: "IC Inversion", cx: 4, cat: "Manipulation", sub: "Control", activation: "1 Action", range: "Touch (Linked)", runtime: "1 scene", link: true,
    text: "Subvert a Linked node's Intrusion Countermeasures. Node rolls Cipher Save vs. your DC at Snag. On failure, the node's IC treats other users (including its owner) as hostile intruders for the duration. Watching a corporate Guardian get Counterattacked by their own network is its own reward." },
  { name: "Deep Sync", cx: 4, cat: "Protection", sub: "Defensive", activation: "1 Swift Action", range: "Self", runtime: "1 scene",
    text: "You sink into a hardened sync state, runtime braced against the feedback you know is coming. While Deep Sync holds, you roll every **Stability Check with Edge**. If you intercept an IC Counterattack, the psychic damage is reduced by your deck's **Firewall Damage Threshold a second time** (in addition to the standard interception reduction). The deck runs cold and quiet while it lasts. You stop flinching at the code." },

  /* ---- Complexity 5 · Apex ---- */
  { name: "Black Sun", cx: 5, cat: "Offense", sub: "Combat", activation: "1 Action", range: "Self (via Link)", runtime: "Instant", link: true, signature: true,
    text: "The legendary cipher. Roll **6d6 damage** and subtract the Linked node's Firewall Damage Threshold. If any damage remains, the node loses **System Integrity equal to half its maximum System Integrity (rounded down)** instead of the rolled damage. A successful Black Sun against a damaged Apex node can brick it in a single strike. Using Black Sun automatically triggers Alert on the node regardless of save outcome. **Signature:** Costs 1 Bandwidth regardless of Complexity." },
  { name: "Soul Transcription", cx: 5, cat: "Manipulation", sub: "Utility", activation: "1 Action (sustained 1 minute)", range: "Touch (Linked)", runtime: "Permanent", link: true,
  ...
  { name: "Override Reality", cx: 5, cat: "Manipulation", sub: "Control", activation: "1 Action", range: "Self (via Link)", runtime: "1 round", link: true,
    text: "For one round, you rewrite the rules of a Linked node. Within that node, you may declare one fundamental change to its function (e.g., \"this turret targets its own faction,\" \"this door is open to me but locked to everyone else,\" \"this security camera shows last week's footage\"). Node rolls Cipher Save vs. your DC. On failure, the change holds for one round. On success, IC triggers Lockdown automatically regardless of IC tier." },
  { name: "Severance", cx: 5, cat: "Protection", sub: "Defensive", activation: "1 Impulse Action", range: "Self", runtime: "Instant",
    text: "When you would suffer **LinkDeath**, when your deck would be **Bricked**, or when **Lockdown** catches you, you cut your consciousness clear of the stream a half-second before the feedback lands. The triggering effect is negated outright: no psychic damage, no System Integrity loss, your Links hold (or, against Lockdown, you disconnect freely and the Lockdown ends). One clean exit, once a scene, for the three worst things the #GRID can do to you." }
];
...
(function () {
  ...
  (EN.grid.ciphers || []).forEach(function (cy) {
    ...
    items.push({
      ...
      effect: cy.cat + " (" + cy.sub + ") · " + cy.activation + " · " + cy.range + (cy.signature ? " · Signature (flat 1 BW)" : "") + (cy.link ? " · needs Link" : ""),
      desc: cy.text
    });
  });
})();
