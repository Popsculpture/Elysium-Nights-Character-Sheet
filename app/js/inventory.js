...
EN.inventoryView = (function () {
  ...
  function workbenchView(ch) {
    ...
    /* '98 and #GRIDroid solid-fill a selected (.primary) button's background with var(--accent)
       and set color:#000 on the class rule, which is the one thing that GUARANTEES the label
       reads against that fill no matter which of the nine color themes is active. The bench's
       own inline color used to override that unconditionally, so a bench whose fixed hue
       happened to land near a theme's own accent (gold on Elysium Nights, green on Slime Time,
       orange on Flavor Wizard...) went invisible when selected. Retinting one bench away from
       one collision only moves the problem to a different theme; there is no fixed color for
       five benches that stays distinct from nine themes' accents. So the override is dropped for
       exactly the skins and the state that solid-fills with the theme's own colour: unselected
       keeps every bench's own colour everywhere, and Classic's selected state (a faint tint, never
       a solid accent fill) is untouched, since it was never the skin with the collision. */
    var flatAccentPrimary = document.documentElement.classList.contains("skin-droid") || document.documentElement.classList.contains("skin-98");
    out.push(el("div.row.wrap", { style: { gap: "6px", marginBottom: "12px" } }, BENCHES.map(function (b) {
      var on = _bench === b.key;
      // a bench with its own svg renders that instead of its text glyph
      var kids = b.svg ? [el("span", { html: b.svg }), document.createTextNode(" " + b.label)] : b.icon + " " + b.label;
      return el("button.btn.sm" + (on ? ".primary" : ""), { style: (on && flatAccentPrimary) ? null : { color: b.color, borderColor: b.color },
  ...
  function render(mount) {
    ...
    // the author's Glimmer money bag, for SPLIT. Drawn nonzero like its source: the bag and the
    // G nested in it rely on it, and evenodd would punch holes he never drew.
    var ICON_SPLIT = '<svg viewBox="0 0 94.56 122.88" fill="currentColor" aria-hidden="true"><path d="M 48.929688 0.005859375 C 42.317188 -0.080390625 35.779844 1.2003906 33.089844 4.1503906 C 34.539844 9.9003906 36.240625 15.550391 38.140625 21.150391 L 31.759766 9.109375 C 28.829766 10.329375 23.269766 13.559062 23.259766 17.289062 C 23.259766 18.209063 23.609141 19.130781 24.369141 20.050781 L 37.25 35.5 C 40.85 33.59 44.729062 32.98 49.539062 34 L 54.160156 29.019531 C 53.440156 30.709531 52.569609 32.579531 51.849609 34.269531 C 53.399609 34.819531 55.000859 35.210469 56.630859 35.480469 L 69.320312 20.699219 C 70.590312 19.219219 70.990547 17.769609 70.810547 16.349609 C 70.530547 14.159609 69.120781 12.580156 67.550781 11.160156 C 66.890781 10.560156 66.189453 10.009531 65.439453 9.5195312 C 63.669453 11.699531 62.669453 13.300937 59.689453 17.210938 L 65.220703 4.0605469 C 62.230703 1.5455469 55.542187 0.092109375 48.929688 0.005859375 z M 44.470703 37.339844 L 36.140625 48.380859 L 41.880859 37.699219 C 35.950859 39.709219 30.969375 44.520078 26.359375 48.580078 C 24.569375 50.180078 22.849688 51.9 21.179688 53.75 C 8.4996875 67.81 -5.4103125 90.780547 2.1796875 110.06055 C 6.8796875 122.01055 24.889531 121.98984 36.519531 122.58984 C 47.339531 123.14984 66.530312 123.00945 76.820312 120.93945 C 93.240313 117.63945 97.149844 103.47984 93.089844 88.339844 C 91.049844 82.089844 88.550938 76.490156 85.710938 71.410156 C 80.140938 61.460156 62.890859 39.570312 52.130859 37.570312 L 54.640625 49.060547 L 48.859375 37.339844 L 44.470703 37.339844 z M 62.246094 60 C 65.866779 60 68.302734 62.172736 68.302734 65.398438 C 68.302734 72.508145 56.914648 84.159743 44.472656 86.792969 C 45.394284 89.228702 47.367699 90.414062 50.330078 90.414062 C 57.308127 90.413854 65.010504 84.094738 67.775391 81.527344 L 68.5 82.25 C 60.666155 90.478832 67.84163 106.67383 45.261719 106.67383 C 34.662986 106.67383 31.964844 101.2103 31.964844 97.787109 C 31.964844 94.627238 34.201806 92.058594 37.625 92.058594 C 39.929072 92.058594 41.707031 93.440871 41.707031 95.613281 C 41.707031 98.378168 39.732486 98.511389 36.835938 99.367188 C 35.716816 99.762172 35.123047 100.42074 35.123047 101.4082 C 35.123047 104.04143 39.994775 105.55469 43.417969 105.55469 C 52.239275 105.55469 52.173861 96.470668 60.666016 87.978516 C 56.716176 90.216757 53.358637 91.335938 50.59375 91.335938 C 47.631371 91.335938 44.866223 89.951811 42.298828 87.121094 C 40.916386 87.186924 39.995971 87.253906 39.535156 87.253906 C 32.491277 87.253906 26.5 82.053992 26.5 75.273438 C 26.5 67.110436 34.794608 61.974119 43.023438 60.130859 L 43.220703 60.986328 C 34.596889 63.092908 29.923828 71.585706 29.923828 78.037109 C 29.923828 82.974407 33.806513 86.265625 39.599609 86.265625 C 40.323747 86.265625 40.983698 86.200596 41.576172 86.134766 C 40.457052 84.159847 39.863281 81.986515 39.863281 79.550781 C 39.863281 68.557273 51.779021 60 62.246094 60 z M 63.496094 61.25 C 56.123061 61.25 45.129234 77.641689 44.273438 85.804688 C 57.702887 83.171462 66.919922 69.874099 66.919922 64.410156 C 66.919922 62.501068 65.339351 61.25 63.496094 61.25 z"/></svg>';
    // the author's price list with the Glimmer script G in place of its dollar sign, for BILLS.
    // Two paths, and only the sheet carries fill-rule evenodd, exactly as the source does: the G is
    // drawn nonzero and would fill its own counters if evenodd were hoisted onto the root.
    var ICON_BILLS = '<svg viewBox="0 0 122.88 121.9" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M42.64,63.21a3.43,3.43,0,0,1,0-6.76H80.2a3.43,3.43,0,0,1,0,6.76ZM77.32,112.88a3.39,3.39,0,0,1,0,6.78H7.89A7.89,7.89,0,0,1,0,111.77V7.89A7.89,7.89,0,0,1,7.89,0h96.89a7.89,7.89,0,0,1,7.89,7.89V45.32a3.39,3.39,0,1,1-6.78,0V7.89a1.11,1.11,0,0,0-.33-.78,1.13,1.13,0,0,0-.78-.33H7.89a1.19,1.19,0,0,0-.79.32,1.21,1.21,0,0,0-.32.79V111.76a1.11,1.11,0,0,0,1.11,1.11H77.32ZM42.64,88.08a3.5,3.5,0,0,1,0-6.87H72.17a3.49,3.49,0,0,1,0,6.87Zm0-49.68A3.15,3.15,0,0,1,39.8,35a3.14,3.14,0,0,1,2.84-3.38H80.2A3.15,3.15,0,0,1,83,35,3.14,3.14,0,0,1,80.2,38.4ZM26.35,80a4,4,0,1,1-4,4,4,4,0,0,1,4-4Zm0-24.49a4,4,0,1,1-4,4,4,4,0,0,1,4-4Zm0-24.48a4,4,0,1,1-4,4,4,4,0,0,1,4-4Z"/><g transform="translate(71.91738249 71.98246922) scale(0.0125283311)"><path transform="matrix(1.739866 0 0 1.739866 -934.117779 -2494.724795)" d="M1961.2716 2850.5089 C1818.8962 2850.5089 1723.9792 2793.5587 1679.6847 2676.4945 C2277.6614 2549.9386 2825.0157 1989.9286 2825.0157 1648.2277 C2825.0157 1493.1967 2707.9515 1388.788 2533.9371 1388.788 C2030.8773 1388.788 1458.2118 1800.0948 1458.2118 2328.4557 C1458.2118 2445.5199 1486.6869 2549.9286 1540.4731 2644.8455 C1511.9981 2648.0094 1480.3591 2651.1733 1445.5562 2651.1733 C1167.1332 2651.1733 980.46321 2492.9784 980.46321 2255.6861 C980.46321 1945.6241 1205.10001 1537.4812 1619.57061 1436.2365 L1610.07891 1395.1058 C1214.59171 1483.695 815.94052 1730.479 815.94052 2122.8024 C815.94052 2448.6838 1103.8552 2698.6318 1442.3923 2698.6318 C1464.5396 2698.6318 1508.8342 2695.4679 1575.276 2692.304 C1698.668 2828.3516 1831.5518 2894.7935 1973.9272 2894.7935 C2106.8109 2894.7935 2268.1697 2841.0072 2458.0036 2733.4347 C2049.8607 3141.5775 2053.0246 3578.1954 1629.0623 3578.1954 C1464.5396 3578.1954 1230.4111 3505.4258 1230.4111 3378.8698 C1230.4111 3331.4114 1258.8862 3299.7724 1312.6725 3280.789 C1451.884 3239.6583 1546.8009 3233.3305 1546.8009 3100.4468 C1546.8009 2996.0382 1461.3757 2929.5963 1350.6393 2929.5963 C1186.1166 2929.5963 1078.544 3052.9884 1078.544 3204.8555 C1078.544 3369.3781 1208.2639 3631.9817 1717.6514 3631.9817 C2802.8684 3631.9817 2458.0036 2853.6628 2834.5074 2458.1755 L2799.7045 2423.3727 C2666.8208 2546.7647 2296.6448 2850.4989 1961.2716 2850.5089 Z M2758.5739 1600.7692 C2758.5739 1863.3727 2315.6281 2502.4801 1670.193 2629.036 C1711.3236 2236.7127 2239.6946 1448.9021 2594.0512 1448.9021 C2682.6403 1448.9021 2758.5739 1509.0162 2758.5739 1600.7692 Z"/></g></svg>';
    ...
    function subTab(key, label) {
      ...
      return el("button.btn.sm" + (_sub === key ? ".primary" : ""), { onclick: function () { _sub = key; EN.app.render(); } }, kids);
    }
    blocks.push(el("div.row.between.wrap.inv-bar", { style: { gap: "10px", marginBottom: "12px", alignItems: "center",
      ...
      el("div.row.wrap", { style: { gap: "6px" } }, [
        subTab("stash", EN.app.iconArchive ? [EN.app.iconArchive, "STASH"] : "▣ STASH"),
        subTab("chrome", [ICON_CHROME, "CHROME"]),
      ...
      el("div", { style: { display: "flex", flexDirection: "column", gap: "5px", alignItems: "flex-end" } }, [
        ...
        el("div.row.wrap", { style: { gap: "6px", alignItems: "center", justifyContent: "flex-end" } }, [
          el("button.btn.sm" + (_split.open ? ".primary" : ""), { title: "Split a contract payout: fixer's cut off the top, then even shares",
            onclick: function () { _split.open = !_split.open; EN.app.render(); } },
            [el("span", { html: ICON_SPLIT }), document.createTextNode(" SPLIT")]),
          el("button.btn.sm" + (_hh.open ? ".primary" : ""), { title: "Bills: lifestyle, safehouse, leases, Hypercare, licences and debts",
            ...
            (billsOverdue(ch) ? [document.createTextNode("⚠ ")] : [])
              .concat([el("span", { html: ICON_BILLS }), document.createTextNode(" BILLS")]))
        ])
      ])
    ]));
