...
title bar, #bdbdbd face grey, #747474 desktop, black text. Two asks, and the second one turned out
to be three separate causes rather than one.

**The gradient, which is the ask that was actually about taste.** '98's title bar ran
`accent-dim` into `--bg1`, so a navy bar faded into window grey. Windows 98's own runs colour into
LIGHTER COLOUR, #000080 into #1084D0, which is what the author asked for and sent a reference of.
`--accent-hi` is now derived per palette in `applyVars` and used as the gradient's far end. It is
derived by raising HSL lightness while holding hue and nudging saturation, NOT by mixing toward
white, because mixing toward white lands back on grey and undoes the request. On this palette
#000582 becomes #1c25ff. `--chrome-b` follows it, so the paper sub-windows match.

**Then the text, which was measured rather than eyeballed.** A contrast sweep over 534 rendered
strings on this palette found 21 kinds under 3:1, and they split three ways.

*The app's semantic colours are dark-theme constants.* `--ember` scored 1.00 against the panel,
`--flow` 1.11, `--danger` 1.14, `--success` 1.16. These are palette-independent literals tuned for
dark grounds. The fix already existed in the file: `html.skin-98 :is(.inv-sub, .post-sub)` has
carried a full light-ground remap for the paper sub-views since they were built. Those values are
now lifted to the whole view whenever the palette is light, off a new `pal-light` class that
`applyVars` sets from the luminance of the panel ground. Ember, gold, success and warn run darker
here than in the paper views on purpose: those sit on white, a '98 window is #bdbdbd, and the same
ink loses about a third of its contrast on it. The paper values gave 2.83 and 2.89 on grey; these
clear 4.5.

*The desktop outline was built for the opposite palette.* Loose labels sit straight on the
wallpaper, and the tray's TEXT OUTLINE draws a black stack around them, which is the Windows 98
desktop-icon convention and right for every dark palette. A light palette writes those labels in
near-black, so a black outline round black text is nothing at all and the words vanish on a bright
sky. `.pal-light` now inverts that stack to white.

*And one cause is structural, so it is reported rather than patched.* Of the 248 low-contrast
elements left, **82 are one thing: `--accent` used as INK.** This palette puts #d9d9d9 in the
accent slot, which is correct for a Windows 98 face colour and wrong for the job the app gives it,
since `--accent` is simultaneously the ink for stat values, chips, buttons and labels AND the fill
for selected states. No palette can satisfy both on a light scheme. The honest fix is not a colour
tweak: on '98 the title bar should arguably read `--accent` rather than `--accent-dim`, which would
let the navy sit in the accent slot and be both the title bar and the readable ink, exactly as
Windows used one highlight colour for both. That changes the skin's mapping for every existing '98
palette, so it is the author's call and is not being made unasked.

Verified on the author's own scheme: the bar renders rgb(0,5,130) into rgb(28,37,255) with white
title text legible across its whole width, and the low-contrast kinds fall from 21 to 15, the
remainder being the accent-as-ink cluster above.
